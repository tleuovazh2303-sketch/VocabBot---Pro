...
# --- 1. RENDER ҮШІН ВЕБ-СЕРВЕР (Health Check тексерісінен өту үшін) ---
web_app = Flask('')
...
# Wordwall алынып тасталған мәзір
...
async def handle_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Learn Words 📚":
        ...
        await update.message.reply_text(f"🎮 *Unscramble:* {shuffled}")
        return

    elif text == "Sentence ✍️":
        w = get_unique_item(context, WORDS_DB, "words")["word"]
        context.user_data.update({"state": "SENTENCE", "target": w})
        await update.message.reply_text(f"✍️ Write a sentence using: *{w}*", parse_mode="Markdown")
        return
        ...
        await update.message.reply_voice(voice=audio_stream, caption="🎧 Listen and type:")
        return
    ...
    if state == "QUIZ":
        ...
        if text.strip().upper() == context.user_data.get("ans", ""):
            ...
            await update.message.reply_text(f"❌ Wrong. It was: {context.user_data.get('ans')}")
        context.user_data["state"] = None
    elif state == "SENTENCE":
        ...
        if target in text.lower() and len(text.split()) >= 3:
            ...
            await update.message.reply_text(f"⚠️ Use '{target}' in a full sentence.")
        context.user_data["state"] = None
